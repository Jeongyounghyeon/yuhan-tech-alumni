import Link from "next/link";
import { prisma } from "@/lib/prisma";
import { formatDate } from "@/lib/utils";
import { Pagination } from "@/components/features/Pagination";

export const dynamic = "force-dynamic";

export const metadata = { title: "자유게시판 | 유한공업고등학교 총동문회" };

const LIMIT = 10;

export default async function BoardPage({
  searchParams,
}: {
  searchParams: Promise<{ page?: string }>;
}) {
  const { page: pageParam } = await searchParams;
  const page = Math.max(1, Number(pageParam ?? 1));
  const skip = (page - 1) * LIMIT;

  const [posts, total] = await Promise.all([
    prisma.post.findMany({
      orderBy: { createdAt: "desc" },
      skip,
      take: LIMIT,
      select: {
        id: true,
        title: true,
        viewCount: true,
        createdAt: true,
        author: { select: { name: true } },
        _count: { select: { comments: true } },
      },
    }),
    prisma.post.count(),
  ]);

  const totalPages = Math.ceil(total / LIMIT);

  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <div className="flex items-end justify-between mb-2">
        <h1 className="section-title">자유게시판</h1>
        <Link
          href="/board/new"
          className="text-sm bg-primary text-primary-foreground px-4 py-2 rounded-lg hover:bg-primary/90 transition-colors"
        >
          글쓰기
        </Link>
      </div>
      <div className="section-divider" />

      <div className="border border-border rounded-lg overflow-hidden">
        <div className="grid grid-cols-12 gap-2 px-4 py-2.5 bg-muted/50 text-xs font-semibold text-muted-foreground border-b border-border">
          <span className="col-span-1 text-center">번호</span>
          <span className="col-span-6">제목</span>
          <span className="col-span-2 text-center">작성자</span>
          <span className="col-span-2 text-center">날짜</span>
          <span className="col-span-1 text-center">조회</span>
        </div>

        {posts.length === 0 ? (
          <p className="text-center text-muted-foreground py-12 text-sm">등록된 게시글이 없습니다.</p>
        ) : (
          posts.map((p, i) => (
            <Link
              key={p.id}
              href={`/board/${p.id}`}
              className="grid grid-cols-12 gap-2 px-4 py-3 border-b border-border last:border-0 hover:bg-accent/50 transition-colors items-center"
            >
              <span className="col-span-1 text-center text-sm text-muted-foreground">
                {total - skip - i}
              </span>
              <span className="col-span-6 text-sm font-medium truncate">
                {p.title}
                {p._count.comments > 0 && (
                  <span className="ml-1.5 text-primary text-xs">[{p._count.comments}]</span>
                )}
              </span>
              <span className="col-span-2 text-center text-xs text-muted-foreground">{p.author.name}</span>
              <span className="col-span-2 text-center text-xs text-muted-foreground">{formatDate(p.createdAt)}</span>
              <span className="col-span-1 text-center text-xs text-muted-foreground">{p.viewCount}</span>
            </Link>
          ))
        )}
      </div>

      <Pagination page={page} totalPages={totalPages} />
    </div>
  );
}
